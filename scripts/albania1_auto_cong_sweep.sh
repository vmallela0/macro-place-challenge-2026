#!/bin/bash
# albania1 AUTO_CONG sweep on the 5 highest-room benches.
#
# Validated on ibm12: weight=1.5 (vs default 0.5) gave 1.7× more
# improvement (-0.0099 vs -0.0062 vs verified). Run on the other 4
# high-room benches to confirm pattern + accumulate mean improvement.
#
# Benches (residual order):
#   ibm12 (+0.269) — re-running for consistency check
#   ibm06 (+0.262)
#   ibm18 (+0.234)
#   ibm07 (+0.080)
#   ibm03 (+0.062)
...
OUT="/tmp/albania1_auto_cong_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$OUT"
...
# THE KEY: enable per-bench AUTO_CONG scaling
export PLACER_V7_HESSIAN_CONG=1
export PLACER_V7_HESSIAN_AUTO_CONG=1
export PLACER_V7_HESSIAN_CONG_WEIGHT=0.5   # base; AUTO scales by 0.5/1.5/2/3
export PLACER_V7_K_CONG_FRAC=0.05
export PLACER_V7_HESSIAN_HPWL_WEIGHT=1.0
export PLACER_V7_HESSIAN_DENS_WEIGHT=0.5

# 5 highest-room benches per residual table
BENCHES="ibm06 ibm18 ibm07 ibm03 ibm12"

echo "albania1 AUTO_CONG sweep on 5 high-room benches" > "$OUT/sweep.log"
echo "  started: $(date)" >> "$OUT/sweep.log"
echo "  results dir: $OUT" >> "$OUT/sweep.log"
echo "  AUTO_CONG=1, base cong_weight=0.5, scaled per-bench by residual" >> "$OUT/sweep.log"
...
for b in $BENCHES; do
  ...
  echo "  $b AUTO_CONG proxy=${proxy:-NA} (verified=$base, Δ=$delta) cong=${cong:-NA} wall=${elapsed}s" \
    | tee -a "$OUT/sweep.log"
done

echo "" >> "$OUT/sweep.log"
echo "albania1 AUTO_CONG sweep finished: $(date)" >> "$OUT/sweep.log"
