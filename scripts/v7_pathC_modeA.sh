#!/bin/bash
# Path C+ Mode A: HPWL + Density Adam (congestion DISABLED).
# Triggered if Path C+ (full-surrogate) hard benches all reject Adam,
# proving the congestion-surrogate divergence is structural for v7.
#
# Mode A drops the congestion term entirely — surrogate becomes:
#   smooth_proxy = HPWL_lse + 0.5 · CVaR_dens(top-K_d)
# HPWL and density are geometrically defined by coordinates; no frozen
# routing lie. If Mode A produces ADAM WIN on hard benches, the
# congestion surrogate is the only liar in the room.
...
OUT="$RESULTS_DIR_BASE/v7_pathC_modeA_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$OUT"
...
export PLACER_V7_ADAM_ENABLE_CONG=0       # ← Mode A: drop the congestion liar
...
echo "v7 Path C+ Mode A (HPWL + Density Adam, no congestion)" > "$OUT/sweep.log"
echo "  started: $(date)" >> "$OUT/sweep.log"
echo "  results dir: $OUT" >> "$OUT/sweep.log"
echo "  Adam: steps=${PLACER_V7_ADAM_STEPS} K_d=${PLACER_V7_K_DENS_FRAC}" >> "$OUT/sweep.log"
echo "  Surrogate: HPWL + Density only (congestion disabled)" >> "$OUT/sweep.log"
echo "" >> "$OUT/sweep.log"
...
echo "" >> "$OUT/sweep.log"
echo "v7 Path C+ Mode A finished: $(date)" >> "$OUT/sweep.log"
