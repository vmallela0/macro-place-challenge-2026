#!/bin/bash
# Overnight v7 sweep across all 17 IBM benchmarks.
# Same harness as scripts/v6_overnight_sweep.sh but pointed at the v7
# placer.py (which is v6 + Laplacian + basin-hopping).
#
# Per-bench output: <OUT>/<bench>.log, <OUT>/<bench>.png, <OUT>/<bench>.npy
# Hard benches (proxy >= 1.0) get a diagnostic GIF after main sweep ends.
...
OUT="$RESULTS_DIR_BASE/v7_overnight_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$OUT"
...
export PLACER_V7_BASIN_HOPS=0     # auto-trigger; only fires on hard benches
export PLACER_V7_BASIN_SIGMA0=0.10
...
echo "v7 overnight sweep" > "$OUT/sweep.log"
...
echo "" >> "$OUT/sweep.log"
echo "v7 main sweep finished: $(date)" >> "$OUT/sweep.log"
...
# Diagnostic GIFs for hard benches.
echo "" >> "$OUT/sweep.log"
...
if [ -z "$HARD_BENCHES" ]; then
  echo "  (no benches >= 1.0)" | tee -a "$OUT/sweep.log"
else
  for hb in $HARD_BENCHES; do
    ...
    .venv/bin/python scripts/make_v6_gif.py "$hb" "$GIF_BUDGET" \
      "assets/v7_${hb}.gif" >> "$OUT/sweep.log" 2>&1 || \
      echo "    gif assets/v7_${hb}.gif failed" | tee -a "$OUT/sweep.log"
  done
fi
