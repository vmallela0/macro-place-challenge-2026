"""exp_v76: 16-direction soft CD (twice fine-grained angular resolution)."""
import sys
...
def soft_cd_16dir(pos_np, benchmark, incr_eval, max_time, verbose=False):
    ...
    dirs = [(np.cos(a), np.sin(a)) for a in np.linspace(0, 2*np.pi, 16, endpoint=False)]
    ...
    if verbose:
        print(f"    16dir: {n_moves} moves, cost {best_cost:.6f}")
    return pos_np, best_cost


import _softmacro
_softmacro.soft_macro_cd = soft_cd_16dir
_m.soft_macro_cd = soft_cd_16dir
