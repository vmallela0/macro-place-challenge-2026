"""exp_v46: v36 with EXPLORATION-HEAVY settings for sparse benchmarks.

Larger deltas in hard CD, more LNS destroy size, more candidates.
"""
import sys
import time
import random
...
class OptimalPlacer:
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        ...
        # EXPLORATION-heavy LNS: bigger n_destroy, MORE candidates
        _, c = lns_destroy_repair_phase(best_pos, benchmark, incr_eval,
                                        max_time=20, n_destroy=10,  # bigger
        ...
        while time.time() - t0 < self.TOTAL_BUDGET - 5:
            ...
            try:
                _, c = fd_soft_place(best_pos, benchmark, incr_eval,
                                     max_time=cycle_t * 0.05, damping=0.5, check_cost=True)
                if c < best_cost: best_cost = c
            ...
            try:
                ...
                _, c = soft_lns_phase(best_pos, benchmark, incr_eval,
                                      max_time=cycle_t * 0.4, n_destroy=15,  # bigger
                                      n_candidates=50)
                if c < best_cost: best_cost = c
            except Exception: pass
