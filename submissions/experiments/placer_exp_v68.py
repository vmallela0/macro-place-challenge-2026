"""exp_v68: v51 with per-net refresh every cycle."""
import sys
import time
import random
...
class OptimalPlacer:
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        ...
        try:
            _, c = per_net_optimize(best_pos, benchmark, incr_eval, max_time=15)
        ...
        while time.time() - t0 < self.TOTAL_BUDGET - 5:
            cycle += 1
            # Per-net refresh every 3rd cycle
            if cycle % 3 == 0:
                try:
                    _, c = per_net_optimize(best_pos, benchmark, incr_eval,
                                             max_time=cycle_t * 0.15)
                    if c < best_cost: best_cost = c
                except Exception: pass
            try:
            ...
            try:
                _, c = soft_macro_cd(best_pos, benchmark, incr_eval, max_time=cycle_t * 0.35)
                if c < best_cost: best_cost = c
            except Exception: pass
            try:
                _, c = soft_lns_phase(best_pos, benchmark, incr_eval,
                                      max_time=cycle_t * 0.3, n_destroy=8, n_candidates=30)
                if c < best_cost: best_cost = c
            except Exception: pass
