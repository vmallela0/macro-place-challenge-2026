"""exp_v2: LNS + CD interleaving. Drop cluster/grad phases. Bet on what worked.

Pipeline:
  1-3. push-apart + legalize + CD1 (125s)
  4. LNS round A (20s)
  5. CD polish A (10s)
  6. LNS round B (15s)
  7. CD polish B (10s)
...
class OptimalPlacer:
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        ...
        # Alternating LNS + CD rounds
        for phase_name, lns_time, cd_time in [
            ("A", self.LNS_A, self.CD_A),
            ("B", self.LNS_B, self.CD_B),
            ("C", self.LNS_C, self.CD_C),
        ]:
            lns_start = time.time()
            try:
                ln_pos, ln_cost = lns_destroy_repair_phase(
                    best_pos, benchmark, incr_eval, max_time=lns_time, verbose=True)
                if ln_cost < best_cost:
                    best_cost = ln_cost
                    best_pos = ln_pos
            except Exception as e:
                print(f"    lns error: {e}")
            print(f"  [lns_{phase_name}] {time.time() - lns_start:.1f}s cost={best_cost:.6f}")

            cd_start = time.time()
            cd_pos, cd_cost = _coord_descent(
                best_pos, benchmark, plc_cd, max_time=cd_time, incr_eval=incr_eval)
            if cd_cost < best_cost:
                best_cost = cd_cost
                best_pos = cd_pos
            print(f"  [cd_{phase_name}] {time.time() - cd_start:.1f}s cost={best_cost:.6f}")
