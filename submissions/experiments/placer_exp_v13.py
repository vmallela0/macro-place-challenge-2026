"""exp_v13: spectral init added to legalization tournament."""
import sys
import time
import random
...
from _spectral_init import spectral_init
...
class OptimalPlacer:
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        ...
        s = time.time()
        try:
            spectral_pos = spectral_init(benchmark, plc, damping_to_init=0.3)
            print(f"  [spectral] {time.time() - s:.1f}s")
        ...
        if spectral_pos is not None:
            # Push-apart the spectral init (it will have overlaps)
            spectral_pushed = _push_apart(spectral_pos, benchmark, max_iters=500, damping=0.6)
            starts.append((spectral_pushed, "spectral"))
        ...
        for ot in range(30):
            ...
            for sm in [0.05, 0.08, 0.12, 0.18]:
                ...
                for sp, name in starts:
                    ...
                    prev_cost = best_cost
                    _try(refined)
                    if best_cost < prev_cost and name == "spectral":
                        print(f"  [spectral WIN] cost={best_cost:.6f}")
        ...
        while time.time() - t0 < self.TOTAL_BUDGET - 5:
            ...
            try:
                _, c = fd_soft_place(best_pos, benchmark, incr_eval,
                                     max_time=self.FD_PER_CYCLE, damping=0.3)
                if c < best_cost:
                    best_cost = c
            except Exception:
                pass
            ...
            try:
                _, c = soft_macro_cd(best_pos, benchmark, incr_eval,
                                     max_time=self.SOFT_CD_PER_CYCLE)
                if c < best_cost:
                    best_cost = c
            except Exception:
                pass
            ...
            try:
                _, c = soft_lns_phase(best_pos, benchmark, incr_eval,
                                      max_time=self.SOFT_LNS_PER_CYCLE,
            ...
            if c < best_cost:
                best_cost, best_pos = c, p
            print(f"  [cycle_{cycle}] cost={best_cost:.6f}")
