"""FINAL SUBMISSION CANDIDATE V3: adaptive cycle budget (v36 winner) + surrogate.

Key win: cycle duration shrinks when plateau, grows when strong gain.
This is v36 + stateful MLP surrogate + full legalize pipeline.
"""
import os
import sys
import time
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        ...
        while time.time() - t0 < soft_deadline:
            ...
            if gain < 5e-5:
                ...
                if plateau >= 4:
                    print(f"  [hard plateau] stop at cycle {cycle}", flush=True)
                    break
            else:
                plateau = 0
                if gain > 0.01:
