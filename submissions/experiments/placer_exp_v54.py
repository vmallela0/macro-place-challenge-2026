"""exp_v54: Harmonic oscillator init for softs, then adaptive CD."""
import sys
import time
import random
...
from _harmonic import harmonic_init_softs
...
class OptimalPlacer:
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        ...
        # Harmonic init for softs (fast, before cycles)
        try:
            saved_softs = incr_eval.macro_pos.copy()
            c_before = incr_eval.get_proxy_cost()
            c_harm = harmonic_init_softs(benchmark, incr_eval, iterations=30)
            if c_harm < c_before - 0.01:
                best_cost = c_harm
                print(f"  [harmonic] {c_before:.6f} -> {c_harm:.6f}")
            else:
                # Revert if not better
                incr_eval.macro_pos[:] = saved_softs
                ...
                print(f"  [harmonic] reverted (gained {c_before - c_harm:.4f})")
