"""exp_v4: v2 + targeted LNS on congestion hotspots + soft macro co-opt.

Pipeline:
  1-2. push-apart + legalize (45s)
  3. CD1 (50s)
  4. Random LNS (15s) + CD (8s)
  5. Targeted LNS on congestion hotspots (15s) + CD (8s)
  6. Random LNS (10s) + CD (8s)
  7. Soft macro CD polish (10s)
  8. Final CD (15s)
  Total ~184s.
"""
import sys
...
class OptimalPlacer:
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        ...
        # Random LNS C with larger n_destroy for diversification
        s = time.time()
        p, c = lns_destroy_repair_phase(best_pos, benchmark, incr_eval,
        ...
        try:
            _, c = soft_macro_cd(best_pos, benchmark, incr_eval,
                                 max_time=self.SOFT_BUDGET, verbose=True)
            if c < best_cost:
                best_cost = c
        except Exception as e:
            print(f"    soft error: {e}")
        print(f"  [soft] {time.time() - s:.1f}s cost={best_cost:.6f}")

        # Final CD polish (hard only — re-sync since soft moves are in incr_eval)
        s = time.time()
        p, c = _coord_descent(best_pos, benchmark, plc_cd, max_time=self.FINAL_CD,
                              incr_eval=incr_eval)
        if c < best_cost:
            best_cost, best_pos = c, p
        print(f"  [final_cd] {time.time() - s:.1f}s cost={best_cost:.6f}")
