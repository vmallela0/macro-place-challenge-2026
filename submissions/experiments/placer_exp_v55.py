"""exp_v55: reversed-order soft CD (LEAST connected first)."""
import sys
...
def soft_cd_reversed(pos_np, benchmark, incr_eval, max_time, verbose=False):
    """Visit LOW-connectivity softs first — fast to move, lots of gain initially."""
    ...
    # REVERSED order: least connected first
    ...
    if verbose:
        print(f"    reversed: {n_moves} moves, cost {best_cost:.6f}")
    return pos_np, best_cost


import _softmacro
_softmacro.soft_macro_cd = soft_cd_reversed
_m.soft_macro_cd = soft_cd_reversed
