"""exp_v71: heavy per-net focus — 3 per-net passes interleaved with soft CD."""
import sys
import time
import random
...
class OptimalPlacer:
    def __init__(self, seed=42):
        self.seed = seed; self.TOTAL_BUDGET = 220; self.LEGALIZE_BUDGET = 75
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        ...
        if plc is None: return benchmark.macro_positions.clone()
        ...
        def _has_overlap(pn):
            for i in range(n_hard):
                for j in range(i + 1, n_hard):
                    if (abs(pn[i, 0] - pn[j, 0]) < (sizes_np[i, 0] + sizes_np[j, 0]) / 2 and
                        abs(pn[i, 1] - pn[j, 1]) < (sizes_np[i, 1] + sizes_np[j, 1]) / 2):
                        return True
            return False

        def _try(pn):
            nonlocal best_pos, best_cost
            if _has_overlap(pn): return
            full = benchmark.macro_positions.clone()
            full[:n_hard] = torch.tensor(pn, dtype=torch.float32)
            r = compute_proxy_cost(full, benchmark, plc_eval)
            if r["overlap_count"] == 0 and r["proxy_cost"] < best_cost:
                best_cost = r["proxy_cost"]; best_pos = pn.copy()
        ...
        for ot in range(30):
            ...
            for sm in [0.05, 0.08, 0.12, 0.18]:
                ...
                for sp, _ in starts:
                    ...
                    seen.add(h)
                    _try(refined)
        if best_pos is None: return benchmark.macro_positions.clone()
        ...
        # THREE per-net passes
        for k in range(3):
            try:
                _, c = per_net_optimize(best_pos, benchmark, incr_eval, max_time=12)
                if c < best_cost: best_cost = c
            except Exception: pass

        _, c = lns_destroy_repair_phase(best_pos, benchmark, incr_eval,
                                        max_time=10, n_destroy=5, n_candidates=50)
        if c < best_cost: best_cost = c
        _, c = _coord_descent(best_pos, benchmark, plc_cd, max_time=5, incr_eval=incr_eval)
        if c < best_cost: best_cost = c

        cycle = 0; cycle_t = 25; last_cost = best_cost
        while time.time() - t0 < self.TOTAL_BUDGET - 5:
            ...
            if gain < 1e-4: cycle_t = max(8, cycle_t * 0.7)
            elif gain > 0.01: cycle_t = min(40, cycle_t * 1.1)
            last_cost = best_cost
