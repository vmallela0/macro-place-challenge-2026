"""exp_v36: adaptive cycle budget — shrink cycle duration when plateau."""
import sys
import time
import random
...
class OptimalPlacer:
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        ...
        # Adaptive cycles — time shrinks if no progress
        cycle = 0
        cycle_t = 30  # start big
        last_cost = best_cost
        while time.time() - t0 < self.TOTAL_BUDGET - 5:
            ...
            print(f"  [cycle_{cycle}] t={cycle_t:.1f}s cost={best_cost:.6f}", flush=True)

            # Shrink/grow cycle duration
