"""exp_v64: per-net + worst-net + adaptive — triple combo."""
import sys
import time
import random
...
class OptimalPlacer:
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        ...
        try:
            _, c = per_net_optimize(best_pos, benchmark, incr_eval, max_time=10)
            if c < best_cost: best_cost = c
        except Exception: pass
        try:
            _, c = worst_net_soft(best_pos, benchmark, incr_eval, max_time=10)
            if c < best_cost: best_cost = c
        except Exception: pass
