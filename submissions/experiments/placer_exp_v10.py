"""exp_v10: v7 cycles + soft-LNS interleaved.

At each soft cycle, alternate: soft CD → soft LNS → FD → hard CD.
The soft LNS finds multi-soft coupled moves that per-macro soft CD misses.
"""
import sys
import time
import random
        ...
        while time.time() - t0 < self.TOTAL_BUDGET - 5:
            ...
            try:
                _, c = soft_macro_cd(best_pos, benchmark, incr_eval,
                                     max_time=self.SOFT_CD_PER_CYCLE, verbose=False)
                if c < best_cost:
                    best_cost = c
            except Exception as e:
                print(f"    softcd error: {e}")
            if time.time() - t0 > self.TOTAL_BUDGET - 5:
                break
            ...
            print(f"  [cycle_{cycle}] {time.time() - s:.1f}s cost={best_cost:.6f}")
