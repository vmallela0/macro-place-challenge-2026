"""exp_v23: soft big-start before cycles (one-shot net-centroid snap)."""
import sys
import time
import random
...
from _soft_bigstart import soft_bigstart
...
class OptimalPlacer:
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        ...
        # Soft big-start before first soft cycle
        s = time.time()
        _, c = soft_bigstart(best_pos, benchmark, incr_eval, verbose=True)
        if c < best_cost:
            best_cost = c
        print(f"  [bigstart] {time.time() - s:.1f}s cost={best_cost:.6f}", flush=True)

        s = time.time()
        p, c = lns_destroy_repair_phase(best_pos, benchmark, incr_eval,
                                        max_time=self.LNS_BUDGET, n_destroy=5, n_candidates=50)
        if c < best_cost:
        ...
        while time.time() - t0 < self.TOTAL_BUDGET - 5:
            ...
            try:
                _, c = soft_macro_cd(best_pos, benchmark, incr_eval, max_time=self.SOFT_CD_PER_CYCLE)
                if c < best_cost: best_cost = c
            except Exception: pass
            if time.time() - t0 > self.TOTAL_BUDGET - 5: break
            try:
