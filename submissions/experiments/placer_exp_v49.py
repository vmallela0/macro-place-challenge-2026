"""exp_v49: reactive phase scheduler — budget follows what's working."""
import sys
import time
import random
...
from _reactive_cycle import ReactiveCycler


def _fd(pos, bench, incr, b):
    return fd_soft_place(pos, bench, incr, max_time=b, damping=0.3, check_cost=True, verbose=False)

def _scd(pos, bench, incr, b):
    return soft_macro_cd(pos, bench, incr, max_time=b, verbose=False)

def _slns(pos, bench, incr, b):
    return soft_lns_phase(pos, bench, incr, max_time=b, n_destroy=8, n_candidates=30, verbose=False)
...
class OptimalPlacer:
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        ...
        while time.time() - t0 < self.TOTAL_BUDGET - 5:
            cycle += 1
            cycle_t = min(30, self.TOTAL_BUDGET - (time.time() - t0) - 5)
            if cycle_t < 10:
                break
            try:
                _, c = cycler.run_cycle(best_pos, benchmark, incr_eval,
                                        total_budget=cycle_t * 0.85, verbose=True)
                if c < best_cost: best_cost = c
        ...
        print(f"  reactive cycles={cycle} final={best_cost:.6f}", flush=True)
