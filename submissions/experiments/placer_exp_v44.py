"""exp_v44: 2-stage cycles — big first, small rest."""
import sys
import time
import random
...
class OptimalPlacer:
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        ...
        # Stage 1: BIG cycles (50s each) x 2
        for i in range(2):
            if time.time() - t0 > self.TOTAL_BUDGET - 30: break
            cycle_t = 50
            try:
            ...
            try:
                _, c = soft_macro_cd(best_pos, benchmark, incr_eval, max_time=cycle_t * 0.5)
                if c < best_cost: best_cost = c
            except Exception: pass
            try:
        ...
        # Stage 2: SMALL cycles (10s each) until end
        while time.time() - t0 < self.TOTAL_BUDGET - 5:
            cycle_t = 10
            try:
                _, c = fd_soft_place(best_pos, benchmark, incr_eval,
                                     max_time=cycle_t * 0.1, damping=0.3, check_cost=True)
                if c < best_cost: best_cost = c
            except Exception: pass
            try:
                _, c = soft_macro_cd(best_pos, benchmark, incr_eval, max_time=cycle_t * 0.5)
            ...
            _, c = _coord_descent(best_pos, benchmark, plc_cd,
                                  max_time=cycle_t * 0.4, incr_eval=incr_eval)
            if c < best_cost: best_cost = c
