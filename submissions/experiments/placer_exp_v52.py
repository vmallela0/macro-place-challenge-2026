"""exp_v52: Langevin MD warm-start added to legalize tournament."""
import sys
import time
import random
...
from _langevin_smooth import langevin_init
...
class OptimalPlacer:
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        ...
        # NEW: Langevin warm-start
        s = time.time()
        try:
            langevin_pos = langevin_init(benchmark, n_steps=200, dt=0.02,
                                          T0=0.5, T_end=0.001, friction=1.0,
                                          alpha=1.0, beta=2.0, verbose=True)
            # push-apart any remaining overlaps
            langevin_push = _push_apart(langevin_pos, benchmark, max_iters=500, damping=0.6)
            print(f"  [langevin] {time.time() - s:.1f}s")
        except Exception as e:
            print(f"  [langevin] FAILED: {e}")
            langevin_push = None
        ...
        if langevin_push is not None:
            starts.append((langevin_push, "langevin"))

        for ot in range(30):
            if time.time() - t0 > self.LEGALIZE_BUDGET: break
            for sm in [0.05, 0.08, 0.12, 0.18]:
                if time.time() - t0 > self.LEGALIZE_BUDGET: break
                for sp, name in starts:
                    if time.time() - t0 > self.LEGALIZE_BUDGET: break
                    ...
                    prev = best_cost
                    _try(refined, name)
                    if best_cost < prev and name == "langevin":
                        print(f"  [langevin WINS!] cost={best_cost:.6f}", flush=True)
