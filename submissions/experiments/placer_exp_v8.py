"""exp_v8: FD soft placement + CD soft. FD for fast convergence, CD for fine-grain.

Pipeline:
  legalize (45s) → cd1 (25s) → LNS (10s) → cd (5s)
  [repeat 5-6x]:
    fd_soft_damped (3s, damping=0.3, check_cost=True)
    soft_cd (8s)
    hard_cd (5s)
  Budget ~180s.
"""
        ...
        while time.time() - t0 < self.TOTAL_BUDGET - 5:
            ...
            try:
                _, c = fd_soft_place(best_pos, benchmark, incr_eval,
                                     ...
                                     check_cost=True, verbose=True)
                if c < best_cost:
                    best_cost = c
            except Exception as e:
                print(f"    fd_{cycle} error: {e}")
            print(f"  [fd_{cycle}] {time.time() - s:.1f}s cost={best_cost:.6f}")

            if time.time() - t0 > self.TOTAL_BUDGET - 5:
                break

            # Soft CD pass (fine-grain)
            s = time.time()
            try:
