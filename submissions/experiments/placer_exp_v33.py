"""exp_v33: cluster-translate softs + regular soft CD."""
import sys
from pathlib import Path
sys.path.insert(0, str(Path(__file__).resolve().parent))
import importlib.util
_s = importlib.util.spec_from_file_location("_v10", str(Path(__file__).resolve().parent / "placer_exp_v10.py"))
_m = importlib.util.module_from_spec(_s); _s.loader.exec_module(_m)

from _cluster_soft import cluster_soft_translate
from _softmacro import soft_macro_cd

def combined(pos_np, benchmark, incr_eval, max_time, verbose=False):
    _, c1 = soft_macro_cd(pos_np, benchmark, incr_eval, max_time * 0.7, verbose=verbose)
    _, c2 = cluster_soft_translate(pos_np, benchmark, incr_eval, max_time * 0.3,
                                    max_cluster_size=6, verbose=verbose)
    return pos_np, min(c1, c2)

import _softmacro
_softmacro.soft_macro_cd = combined
_m.soft_macro_cd = combined


class OptimalPlacer(_m.OptimalPlacer):
    TOTAL_BUDGET = 220
    LEGALIZE_BUDGET = 75
