"""exp_v35: joint hard+soft CD (interleaved hard moves + nearby-soft moves)."""
import sys
import time
import random
...
from _joint_cd import joint_hard_soft_cd
...
class OptimalPlacer:
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        ...
        # Joint phase
        s = time.time()
        _, c = joint_hard_soft_cd(best_pos, benchmark, incr_eval,
                                   max_time=self.JOINT_BUDGET,
                                   softs_per_hard=3, verbose=True)
        if c < best_cost:
            best_cost = c
            # Sync hard positions back
            best_pos = incr_eval.macro_pos[:n_hard].astype(np.float64).copy()
        print(f"  [joint] {time.time() - s:.1f}s cost={best_cost:.6f}", flush=True)

        # Polish
        s = time.time()
        _, c = soft_macro_cd(best_pos, benchmark, incr_eval, max_time=self.SOFT_BUDGET)
        if c < best_cost:
            best_cost = c
        print(f"  [soft] {time.time() - s:.1f}s cost={best_cost:.6f}", flush=True)

        print(f"  [TOTAL] {time.time() - t0:.1f}s final={best_cost:.6f}", flush=True)
